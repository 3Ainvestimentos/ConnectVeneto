...
import { Search, FileText, Download, ChevronDown, ChevronUp, FileType, Folder, BookOpen } from "lucide-react";
...
import {
  formatRepositoryLastModifiedForDisplay,
  formatRepositorySizeForDisplay,
  isAllowedDocumentInternalPath,
  isSafeRepositoryDownloadUrl,
  parseLastModifiedSortValue,
...
export default function DocumentRepositoryClient({
  ...
  const handleOpenDocument = (doc: DocumentType) => {
    ...
    if (isAllowedDocumentInternalPath(doc.internalPath)) {
      logRepositoryAccess(doc, "document_download", {
        contentType: "internal_page",
        internalPath: doc.internalPath!,
      });
      router.push(doc.internalPath!);
      return;
    }

    if (!isSafeRepositoryDownloadUrl(doc.downloadUrl)) {
      toast({
        title: "Link inválido",
        description: "Este documento não possui um URL HTTPS seguro. Contacte o administrador.",
        variant: "destructive",
      });
      return;
    }
      ...
      items = [...items].sort((a, b) => {
        ...
        if (typeof valA === "string" && typeof valB === "string") {
          if (sortKey === "lastModified") {
            comparison = parseLastModifiedSortValue(valA) - parseLastModifiedSortValue(valB);
          } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className="w-full md:w-auto justify-between font-body hover:bg-background hover:text-foreground"
              >
                  ...
                  <TableRow key={doc.id} className="hover:bg-muted/30">
                    ...
                    <TableCell className="font-body max-w-[min(48vw,32rem)]">
                      <button
                        ...
                        className={cn(
                          "block w-full text-left font-medium text-foreground underline-offset-2",
                          "hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:rounded-sm"
                        )}
                        aria-label={
                          isInternal
                            ? `Abrir página: ${doc.name}`
                            : `Abrir ou transferir: ${doc.name}`
                        }
                      >
                        {doc.name}
                      </button>
                    ...
                    <TableCell className="font-body">{formatRepositorySizeForDisplay(doc)}</TableCell>
                    <TableCell className="font-body">{formatRepositoryLastModifiedForDisplay(doc.lastModified)}</TableCell>
                    <TableCell className="text-right">
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleOpenDocument(doc)}
                        aria-label={isInternal ? "Abrir glossário" : "Abrir documento"}
